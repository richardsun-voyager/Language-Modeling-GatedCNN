...
from nlp_blocks.nn import initializer, regularizer, spatial_dropout, get_lstm_init_state, dropout_res_layernorm
...
def TCN_encode(seqs, num_filters_list, scope='tcn_encode_block', reuse=None, **kwargs):
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        for i in range(num_layers):
            ...
            out = Res_DualCNN_encode(outputs[-1], dilation=dilation_size, 
                                     num_filters=num_filters,  scope='res_biconv_%d' % i, **kwargs)
            outputs.append(out)
        return outputs[-1]
