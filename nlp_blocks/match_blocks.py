...
from nlp_blocks.encode_blocks import CNN_encode
from nlp_blocks.nn import linear_logit, layer_norm, dropout_res_layernorm
...
def Attentive_match(context, query, context_mask, query_mask,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        query2context_score = tf.nn.softmax(masked_score, dim=2) * mask  # B, Lc, Lq
        query2context_attention = tf.matmul(query2context_score, query)  # B, Lc, D

        context2query_score = tf.nn.softmax(masked_score, dim=1) * mask  # B, Lc, Lq
